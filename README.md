...
- WorkerはPayPayリンク先を読み取らず、PayPayアカウントへログインもしません。`/approve` の金額は**管理者がPayPay画面で確認して入力する値**です。
- `/approve` は商品に登録された価格と完全一致する整数だけを通します。例：500円の商品は`500`のみ承認でき、1円・499円・501円は停止します。
...
1. `#管理`などの管理チャンネルで `/pending` を実行し、未処理注文と注文番号を確認する。
2. `/claim order:注文番号` を実行する。
3. 自分だけに表示されたリンクをPayPayアプリで開き、商品価格と受取完了を確認する。
4. PayPayで表示された実際の受取額を確認し、`/approve order:注文番号 amount:金額` を実行する。商品価格と一致しない金額はWorker側でも停止します。
5. 誤送信・期限切れは `/cancel order:注文番号` を実行する。

`/buy` と `/download` は販売用チャンネルでのみ実行できます。`/claim`、`/approve`、`/cancel`、`/pending` は設定済みの管理者IDだけが、同じ販売サーバー内の管理チャンネルから実行できます。購入者は `/status order:注文番号` で自分の注文状態だけを確認できます。受取リンクは管理者へ一度だけ表示されます。
...
- `npm run check` が成功し、`/pending`、`/status`、期限切れ注文、金額不一致も確認した。
